...
export default function MostPopular() {
  ...
  return (
    <section
      ...
      className="flex flex-col items-center justify-center gap-y-6 lg:gap-y-12 px-3 w-full overflow-x-hidden"
    >
      {/* Titre et sous-titre */}
        ...
        <div className="flex w-full justify-center lg:justify-between items-center lg:px-7">
          <div className="flex flex-col">
            ...
            <p className="text-center lg:text-start font-light">
              Découvre ici les œuvres les plus populaires de la plateforme !
            </p>
          </div>
          <NavLink
            to="/comics"
            className="hidden lg:flex"
            aria-label="Découvrir plus de comics populaires"
          >
      ...
      <div className="flex flex-col items-center gap-y-4 md:gap-y-12 w-full">
        {/* Bannière panoramique (md et +) */}
        {bannerManga && (
          <article aria-label={`Manga le plus populaire: ${bannerManga.title}`}>
            <NavLink
            ...
            >
              {/* Image de fond floutée décorative */}
              <img
              ...
              <div className="z-20 flex flex-col justify-center items-start gap-4 ml-4 md:ml-8 bg-dark-bg/60 p-4 md:p-6 rounded-lg shadow-xl min-w-[180px] md:min-w-[280px] max-w-[320px] md:max-w-[420px] text-left">
                <h3 className="text-xl md:text-3xl text-accent font-medium tracking-wider">
                  {bannerManga.title}
                </h3>
                <div>
                ...
                {/* Bouton Découvrir, visible uniquement sur desktop */}
                <div className="hidden md:inline-block mt-2">
                  <ButtonAnimated
                    text="Découvrir"
                    onClick={() =>
                      navigate(
                        `/Comics/${bannerManga.id}/${slugify(
                          bannerManga.title
                        )}`
                      )
                    }
        ...
        {/* Version mobile : bannière simple */}
        {bannerManga && (
          <article
            className="w-full md:hidden"
            aria-label={`Manga le plus populaire: ${bannerManga.title}`}
          >
            <NavLink
              to={`/Comics/${bannerManga.id}/${slugify(bannerManga.title)}`}
            ...
            >
              ...
              <div className="absolute bottom-2 left-0 w-full flex justify-center z-20">
                <h3 className="text-2xl text-accent text-center font-medium tracking-wider px-2 py-1 rounded">
                  {bannerManga.title}
                </h3>
              </div>
            </NavLink>
        ...
        {loading ? (
          ...
          <ul
            role="list"
            className="grid grid-cols-3 md:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6 gap-x-3 md:gap-x-4 gap-y-6 w-full max-w-full 2xl:w-[90%] xl:gap-x-12 justify-center"
            aria-label="Mangas populaires"
          >
                  ...
                  >
                    ...
                    >
                      <img
                        ...
                        src={item.coverUrl}
                        alt={`Couverture du manga ${item.title}`}
                        loading="lazy"
                        decoding="async"
                    ...
                    <div className="flex flex-col justify-center items-center text-center w-full">
                      ...
                      <p className="text-xs text-gray-400 md:text-sm line-clamp-2">
                        Auteur : {item.authorName}
                      </p>
                      {item.artistName &&
                        item.artistName !== item.authorName && (
                          <p className="text-xs text-gray-400 md:text-sm line-clamp-2">
      ...
      <nav
        className="lg:hidden"
        aria-label="Navigation vers plus de contenus populaires"
      >
        <NavLink to="/comics">

...
function ForgotPassword() {
  ...
  async function submit(values) {
    console.log(values);
    setFeedbackGood(
      "Un lien vous sera envoyé par mail si l'adresse correspond à un compte valide"
    );
    try {
      await fetch(
        `http://localhost:8000/api/users/changermotdepasse/${values.email}`
      );
    } catch (error) {
      console.error(error);
    }
      ...
      <form onSubmit={handleSubmit(submit)} className={`${styles.formForgot}`}>
        <div className={`${styles.Entête}`}>
          ...
          <span className={`${styles.spanForgot}`}>
            Entre ton mail pour réinitialiser ton mot de passe !
          </span>
        </div>
        <div>

...
router.get("/getChapters/:idComics", (req, res) => {
  console.log(
    "Request received for getChapters with idComics:",
    req.params.idComics
  );
  const idComics = req.params.idComics;
...
router.get("/getImgChapters/:idChapter", (req, res) => {
  console.log(
    "Request received for getImgChapters with idChapter:",
...
router.patch("/addLike/:idComics/:iduser", (req, res) => {
  ...
  connection.query(addLikeSql, [idComics, iduser], (err, result) => {
    ...
    connection.query(updateLikesSql, [idComics], (updateErr, updateResult) => {
      if (updateErr) {
        console.error(
          "Erreur lors de la mise à jour des likes dans comics :",
          updateErr
        );
        res.status(500).send("Erreur interne du serveur");
        return;
...
router.get("/getBooks/:idComics/:iduser", (req, res) => {
  ...
  console.log("iduser bookmarks:", iduser);
...
router.patch("/addBook/:idComics/:iduser", (req, res) => {
  ...
  connection.query(addBookSql, [idComics, iduser], (err, result) => {
    if (err) {
      ...
      res.status(500).send("Erreur interne du serveur addbook");
      return;
    }
    ...
    connection.query(
      ...
      (updateErr, updateResult) => {
        if (updateErr) {
          console.error(
            "Erreur lors de la mise à jour des bookmarks dans comics :",
            updateErr
          );
          res.status(500).send("Erreur interne du serveur updatebook");
          return;
        }
...
router.patch("/removeBook/:idComics/:iduser", (req, res) => {
  ...
  connection.query(removeBookSql, [idComics, iduser], (err, result) => {
    ...
    connection.query(deleteBooksSql, [idComics], (updateErr, updateResult) => {
      if (updateErr) {
        console.error(
          "Erreur lors de la mise à jour des books dans comics :",
          updateErr
        );
        res.status(500).send("Erreur interne du serveur");
        return;
